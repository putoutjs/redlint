...
test('redlint: run-convert-with-options: -h', async (t) => {
    const filesystem = {};
    const [error] = await runDSLConverter(filesystem, ['-h']);
...
test('redlint: run-convert-with-options: --help', async (t) => {
    const filesystem = {};
    const [error] = await runDSLConverter(filesystem, ['--help']);
...
test('redlint: run-convert-with-options: json -> toml: all matched', async (t) => {
    ...
    const [error] = await runDSLConverter(filesystem, [
        'json',
        '->',
        'toml:',
        'package.json',
...
test('redlint: run-convert-with-options: json -> toml: calls convert', async (t) => {
    ...
    await runDSLConverter(filesystem, [
        'json',
        '->',
        'toml:',
        'package.json',
...
test('redlint: run-convert-with-options: missing filename', async (t) => {
    ...
    const [error] = await runDSLConverter(filesystem, ['json', '->', 'toml'], {
        convert,
    });
